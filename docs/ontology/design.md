...
**底座的第一项保证在第一个任务就兑现，与网大不大无关。** 落进网里的断言，`observed` 必须引一个本任务里读入口真发过的地址（E07，第 7 节），地址指回一次真实发生过的读数。没有这条，"AI 是不是没看就说了"这个问题提不出来；有了这条，它有确定答案。网为空时这条同样成立，它是可审计性的全部来源。
...
- 台账按任务隔离，任务结束可弃。它是一次任务内的收据簿，不是存储。
