...
【设计】以上决定推出两句，本文此前未写成条；第 14 节的判据因此测错了对象。

**GMR 不是那张网。** 网是用的人一次次写出来的，它的规模、密度、连通度是使用的产物。GMR 是让它能被建起来的那套东西：记录格式、校验器、四个写入原语、一个标准读入口。网的形状不是 GMR 的交付物，也不是它的验收项。

**底座的第一项保证在第一个任务就兑现，与网大不大无关。** 落进网里的断言，`observed` 必须引一个本任务里读入口真发过的地址（E07，第 7 节）。没有这条，"AI 是不是没看就说了"这个问题提不出来。网为空时这条同样成立。

【设计】这项保证的边界必须写明，否则它会被当成比实际更强的东西：

- **写入那一刻是真强制。** 台账是运行时的事实，本任务没调用过读入口的 agent 台账为空，写任何地址都对不上，整份拒收。
- **事后不自证。** 地址是读数的内容地址（4.4 的 fact address，由领域的地址函数算出），不是一次调用的事件编号。记录事后携带的是依据本身——可重算、可比对、可判 stale——不携带"当时确实调用过读入口"的证据。一个已知源码内容的写入者原则上能自行算出同一地址。
- 因此本文承诺的是：**每条断言都带着一个可重算的依据**，以及**写入时被强制真读过一次**。把强制延伸到事后需要台账留存，或让地址携带不可从内容推导的部分；两者都是新机制，本文不引入，改动走 CLAUDE.md §7。

积累带来的是另一件事：顺着别人留下的边少走弯路。那是本节上一条说的动态平衡，属于使用的结果，且尚未被数据支持（第 14 节【现状】）。两者不可混谈。
...
- 台账按任务隔离。弃掉它即放弃事后重演这次强制的能力，见 §1【设计】对这项保证边界的说明；是否留存由部署定，本文不要求。
