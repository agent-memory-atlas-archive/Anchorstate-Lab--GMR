...
【设计】以上决定推出一句，本文此前未写成条：**GMR 不是那张网。** 网是用的人一次次写出来的，它的规模、密度、连通度是使用的产物。GMR 是让它能被建起来的那套东西：记录格式、校验器、四个写入原语、一个标准读入口。网的形状不是 GMR 的交付物，也不是它的验收项——第 14 节的判据拿网的使用效果当底座的验收，是这条没写下来的后果。
