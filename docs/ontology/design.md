...
| G3 | 断言可依据 (地址, 路径)；同一读数里别的路径变了，这条**不** stale | 纯位移判据，量在本仓库上：给每个 `.rs` 文件顶部插两个空行，不动任何签名或函数体。整地址引用应全线失效，路径引用应一条不动 |

【现状】2026-09-14 跑完，G1–G5 全部通过。G3 的数：567 个 `.rs` 文件各插两个空行后，847 个锚里 750 个换了地址、0 个 state 轴移动；`sync` 从 `watch:` 合成出的 175 条绑定共 594 条依据、1,148 条路径足迹，**整地址形式 581 条报 superseded，路径形式 1,148 条全部 holds，零漏报**。`ground` 的 `Holding` 由整状态 diff 收窄到记录的路径，在一个只改单文件的对照里由 57 条 moved 降到 55 条，同样零漏报——它是第二层、较轻的那一处过度报告。
...
【现状】2026-09-14 量清楚了第二个缺陷住在哪一层：**过度报告在地址层，不在 state 层。**给本仓库每个 `.rs` 文件顶部插两个空行后重新观测——**847 个锚里 750 个换了地址，而移动的 state 轴是 0 个**。coding pack 的 shape 把行号挡在 state 之外（`place` 轴读的 `obs.at.after` 是邻居符号名，不是行号），所以仓库自己的 `check` 与 `ground` 从来没有这个问题；受害的是按整个 fact address 引用的一方——地址覆盖 `facts.line`。Phase 0 的 59→279 与外部部署遇到的，都是这一层。第 4.4 节的路径形式正对这一层。
...
## 19. problems.md 撤下时还开着的问题

【现状】2026-09-14，`docs/problems.md`（2026-09-01）与 `docs/repair-guide.md` 随形状迁进 `packs/coding/shapes` 一并删除。它们是本文之前的问题清单与修复指引，诊断部分已由本文取代——包括 problems.md 写对、而本文一度丢掉又在第 1 节找回的那一条：长期与短期不是两个产品面，是同一个 δ 的两种消费深度。

删除时它仍带着三样本文没有回答的东西。记在这里，好让它们不随文件消失：

- **问一个键要付全仓的价（原第六节）。** 大部分已经不成立了。量在 847 个锚的 release build 上，热态：一个坐标 `read --json` **0.02 s**（原记录 157 ms / 659 锚），全仓 `read --json` **5.6 s**（原 21.4 s），全仓 `status` **6.7 s**（原 22.3 s）。仍然贵的是一个坐标的 `check`：**4.1 s**（原 0.9 s）。它跑探针，做的是真工作，但这个数今天没有任何东西盯着。
- **三份 SKILL.md 没有共同身份（原第七节）。** 未动。第 16 节只说把 SKILL.md 缩成入口卡，没说仓库里的、二进制里 `include_str!` 进去的、和 npm 装出来的那三份怎么归一。
- **四个还没有答案的问题（原第十一节）**：宪法层能压到多小而不失去打断闭环的能力；服务侧在运行时才发现的事实源怎么锚（预先枚举等于退回传统系统，运行时开长期锚则锚随对话无限增长，一次性读数今天不存在）；注入在长会话中是否随上下文增长而失效；"一条笔记提到的符号名是否都在它 `about:` 的覆盖范围内"这类结构问题的召回率有多低。

第二条是外部部署反馈里的同一个问题——运行时才发现要查的那个事实，没有地方安放。它仍然开着，本文没有答案。
